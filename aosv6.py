"""NAPALM Alcatel-Lucent AOSV6 Handler."""
...
try:
    ...
    from napalm_aosv6.utils.AlcatelOSV6 import *
    from napalm_aosv6.utils.utils import *
...
log_level = log_levels.get(os.getenv('NAPALM_AOSV6_LOG_LVL'))
if not log_level:
    log_level = logging.WARNING
logging.basicConfig(level=log_level)


class AOSV6Driver(NetworkDriver):
    """NAPALM Alcatel-Lucent AOSV6 Handler."""
    ...
    def __init__(self,
        ...
        self.candidate_cfg_file = optional_args.get(
            'candidate_cfg_file', 'nap_aosv6-' + str(uuid.uuid4()))
        self.candidate_remote_cfg_file = 'napalm.cfg'
        self.config_replace = False
        self._scp_client = None

        self.device = AlcatelOSV6(hostname, username, password, timeout,
                                optional_args)
    ...
    def open(self):
        ...
        try:
            ...
            self._scp_client = AlcatelOSV6(self.device)
    ...
    def get_facts(self):
        ...
        show_chass = self.device.send_command('show chassis 0')
        print("show_chass")
        print(str(show_chass))
        #show_ip_inf = self.device.send_command(
        #    "show ip interface | awk '{print $1}'")
        
        show_ip_inf_raw = self.device.send_command("show ip interface")
        show_ip_inf_lines = show_ip_inf_raw.strip().split('\n')
        
        # Skip the header lines
        interface_lines = show_ip_inf_lines[3:]
    ...
    def get_interfaces_ip(self):
        ...
        iftable = AOSV6Table(output)
        ifv6table = AOSV6Table(outputv6)
    ...
    def get_interfaces(self):
        ...
        #raw_interfaces_dict = parse_interface_block(output, indent='  ')
        raw_interfaces_dict = parse_interface_block(output)

        command = 'show interfaces status'
        output = self.device.send_command(command)
        iface_status_table = AOSV6Table(output)

        command = 'show interfaces capability'
        output = self.device.send_command(command)
        iface_capability_table = AOSV6Table(output)

        print("RAW DICT KEYS")
        print(raw_interfaces_dict.keys())
    ...
    def get_lldp_neighbors(self):
        """AOSV6 implementation of get_lldp_neighbors."""
        lldp = {}
        command = 'show lldp remote-system'
        output = self.device.send_command(command)
        pattern = r"Slot/Port\s(?P<localport>.*):\n\s*Chassis\s(?P<remotechassis>.*),\sPort\s(?P<remoteport>.*):"
        ...
        for match in matches:
            localport = match.group('localport')
            remote_info = {
                'hostname': match.group('remotechassis'),
                'port': match.group('remoteport')
            }
            lldp[localport] = remote_info

        return lldp

    def get_lldp_neighbors_detail(self, interface=''):
        """
        AOSV6 implementation of get_lldp_neighbors_detail.
        """
        lldp = {}
        command = 'show lldp remote-system'
        output = self.device.send_command(command)
    ...
    def get_ntp_servers(self):
        """Implementation of get_ntp_servers for AOSV6.
    ...
    def get_ntp_peers(self):
        ...
        output = self.device.send_command(command)
        ntp_peers_tbl = AOSV6Table(output)
    ...
    def get_network_instances(self, name=''):
        """get_network_instances implementation for AOSV6."""
        vrfs = {}
        command = 'show vrf'
        output = self.device.send_command(command)
        ...
        for index, vrf_name in enumerate(
            ...
            if name == '' or name == vrf_name:
                ...
                ip_inf_tbl = AOSV6Table(if_output)
    ...
    def get_config(self, retrieve='all', full=False, sanitized=False):
        """Implementation of get_config for AOSV6.

        Returns the startup or/and running configuration as dictionary.
        The keys of the dictionary represent the type of configuration
        (startup or running). Please be aware that AOSV6 doesn't directly support candidate configuration.
    ...
    def get_route_to(self, destination='', protocol='', longer=False):
        ...
        def _get_route_database(destination, route_dict):
            ...
            output = self.device.send_command(command)
            is_active = False
            routes_db_tbl = AOSV6Table(output)
        ...
        for vrf, command_output in zip(vrfs, commands_output):
            routes_tbl = AOSV6Table(command_output)
            for index, ipaddr in enumerate(
    ...
    def get_bgp_neighbors(self):
        ...
        for vrf, ip_bgp_out, router_id_out, bgp_neighbor_out, v6_bgp_neighbor_out in zip(
            ...
            bgp_neighbor_tbl = AOSV6Table(bgp_neighbor_out)
            for index, ipaddr in enumerate(
            ...
            v6_bgp_neighbor_tbl = AOSV6Table(v6_bgp_neighbor_out)
            for index, ipaddr in enumerate(
    ...
    def get_bgp_neighbors_detail(self, neighbor_address=u''):
        ...
        for vrf in vrfs:
            ...
            command = 'vrf {} show ip bgp neighbors'.format(
                vrf)
            bgp_neighbor_tbl = AOSV6Table(self.device.send_command(command))
            neighbors_detail = {}
            for ipaddr in bgp_neighbor_tbl.get_column_by_name("Nbr address"):
                ...
                if neighbor_address == '' or ipaddr == neighbor_address:
                    ...
                    bgp_neighbor_timer_tbl = AOSV6Table(bgp_neighbor_timer_out)
            ...
            command = 'vrf {} show ipv6 bgp neighbors'.format(
                vrf)
            v6_bgp_neighbor_tbl = AOSV6Table(self.device.send_command(command))
    ...
    def get_ipv6_neighbors_table(self):
        ...
        ipv6_neighbors_table = AOSV6Table(ipv6_neighbors)
    ...
    def get_vlans(self):
        ...
        vlantable = AOSV6Table(output)
        iftable = AOSV6Table(outputif)
        ifv6table = AOSV6Table(outputifv6)
